 ...
 * @author med
 */
public class ReclamationController1Admin implements Initializable {
     static int v = 0 ;
     static String t ="";
      static String de ="";
       static String n ="";
 ...
 if (job != null) {
    ...
    private void chercher(ActionEvent event) {
        ...
        try {
            ...
            Logger.getLogger(ReclamationController1Admin.class.getName()).log(Level.SEVERE, null, ex);
        }
        rating.setCellValueFactory(new PropertyValueFactory<>("type"));
        title.setCellValueFactory(new PropertyValueFactory<>("description"));
        category.setCellValueFactory(new PropertyValueFactory<>("user"));
    ...
    private void edit(ActionEvent event) throws IOException  {
            ...
            Optional<ButtonType> action = alert.showAndWait();
            if (action.get() == ButtonType.OK) {
                     Parent root = FXMLLoader.load(getClass().getResource("/GUI/modifier_reclamationAdmin.fxml"));
                     Scene scene = new Scene(root);
                    stageAdd = new Stage();
                      stageAdd.setScene(scene);
                      stageAdd.show();
